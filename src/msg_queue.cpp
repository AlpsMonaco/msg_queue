...
int msg_queue::get_queue_size()
{
	int num = this->queue_size;
	return num;
}
...
{
	int pos = get_next_queue_pos();
	for (;;)
	{
		if (get_unfinished_num() >= queue_size - 1)
...
void msg_queue::done_task()
{
	this->queue_handled_pos++;
}